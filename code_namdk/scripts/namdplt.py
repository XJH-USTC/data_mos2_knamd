...
def main():
    ...
    kplabels = ['K',r'$\Gamma$','M',r'$\Gamma$',r'$\mathrm{K^{\prime}}$']
...
#    print(k_index)
    ...
    if LEF:
        times = np.array([np.arange(EFSTART,NSW,EFTIME),np.arange(EFSTART+1,NSW,EFTIME)]).T.reshape(-1)
        times1 = times
    else:
        ntime = 8
        times = np.arange(EFSTART+1,NSW,NSW/ntime).astype(int)
        times1 = np.arange(EFSTART+1,NSW,NSW/20).astype(int)
        #times = np.arange(EFSTART,NSW,EFTIME)

    if (31 in which_plt):
        plot_tdkprop(kpts, shp, times, lef=LEF, nsw=NSW, nbasis=nbasis, axis='xy', figname='TDKPROPxy.png')
    if (32 in which_plt):
        plot_tdkprop(kpts, shp, times, lef=LEF, nsw=NSW, nbasis=nbasis, axis='yz', figname='TDKPROPyz.png')
    if (33 in which_plt):
        plot_tdkprop(kpts, shp, times, lef=LEF, nsw=NSW, nbasis=nbasis, axis='xz', figname='TDKPROPxz.png')

    if (4 in which_plt):
        plot_tdband_sns(k_loc, en, kp_loc, kplabels, shp, k_index, times,
                        Enk=Enk, Eref=Eref, lef=LEF, nsw=NSW, nbasis=nbasis, 
                        figname='TDBAND.png')

    if (5 in which_plt):
        plot_tdph_sns(q_loc, phen, qp_loc, qplabels, php, q_index, times1,
                      figname='TDPH.png')
...
def plot_tdkprop(kpts, shp, times, lef=False, nsw=None, nbasis=None, axis='xy', figname='TDKPROP.png'):
    ...
    if lef:
        tindex = np.array([times[0::2],times[1::2],times[1::2]]).T.reshape(-1)
        pop = shp[tindex,2:]
        fileshp_pop = glob('fssh_pop_sh1-*.npy')
        shp_pop = np.zeros((nsw,nbasis),dtype=np.float32)
        for i in range(len(fileshp_pop)):
            shp_pop[:] += np.load(fileshp_pop[i])
        shp_pop /= len(fileshp_pop)
        pop[np.arange(1,pop.shape[0],3)] = shp_pop[times[1::2]]
    else:
        tindex = times
        pop = shp[tindex,2:]
    ...
    figsize_x = 3.5 * ncol + 1.0
    figsize_y = 3.6 * nrow # in inches
    ...
    for it in range(nts):
        ...
        ax.set_xlim(0.32, 0.345)
        ax.set_xticks(np.arange(0.32,0.3451,0.005))
        ax.set_ylim(0.32, 0.345)
        ax.set_yticks(np.arange(0.32,0.3451,0.005))
        #ax.set_xlim(0.0, 1.0)
        #ax.set_ylim(0.0, 1.0)
        ax.set_aspect('equal')
        # ax.set_xticks([])
...
def plot_tdband_sns(k_loc, en, kp_loc, kplabels, shp, index, times,
    ...
    if lef:
        ...
        for i in range(len(fileshp_pop)):
            shp_pop[:] += np.load(fileshp_pop[i])
        shp_pop /= len(fileshp_pop)
        pop[np.arange(1,pop.shape[0],3)] = shp_pop[:,index][times[1::2],:]
    else:
        tindex = times
        pop = shp[:,index+2][tindex,:]
    ...
    figsize_x = 2.1 * ncol + 1.0
    figsize_y = 3 * nrow # in inches
    fig, axes = plt.subplots(nrow, ncol)
    fig.set_size_inches(figsize_x, figsize_y)
    mpl.rcParams['axes.unicode_minus'] = False
