...
cdef void KProcSplit(
    ...
    GetKqidx(
        myid,nq,k_list_a,q_list,nqx,nqy,nqz,
...
def ReadH5(
    ...
    cdef int *** k_p_r
    cdef int * k_p_r0
    cdef long l_phzero, l_phonon, nmodes_l
    ...
    KProcSplit(
        c_comm,myid,nprocs,Num,nk_s,nq,nqx,nqy,nqz,nbands,ekidx,ebidx,
...
def ReadNpy(
    ...
    cdef int *** k_p_r
    cdef int * k_p_r0
    cdef long l_phzero, l_phonon, l_epc_t, nmodes_l
    cdef double complex[:,:,::1] epc_a
    ...
    KProcSplit(
        c_comm,myid,nprocs,1,nk_s,nq,nqx,nqy,nqz,nbands,ekidx,ebidx,
        k_range,k_list_a,k_list_a,abc,dt,hbar,EFX,EFY,EFZ,k_proc,
        k_proc_num,nk_proc,kqidx_p,k1qidx_p,k1kidxE,&k_p_r,LEF
    )
    k_list_a = None
...
cdef void TransposePart(
    ...
    for i in range(nmodes):
        for j in range(nk_p):
            for k in range(nk_a):
                epc_a[i,j,k] = epc_t[(j*nk_a+k)*nmodes+i]
    free(epc_t)
...
def ReadNpyPart(
    ...
    cdef int[:,::1] kqidx_p, k1qidx_p, k1kidxE
    cdef long l_phonon, l_epc, l_kqidx, nmodes_l
    cdef double complex[:,:,::1] epc_a
    cdef double starttime, endtime, en, f_r
    cdef int TRANS[3]
