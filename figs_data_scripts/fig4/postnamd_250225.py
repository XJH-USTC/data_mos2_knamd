...
def pick_kpts_on_path(kpts, kpath, A, norm=1e-4):
    ...
    b1, b2, b3 = calc_rec_vec(a1, a2, a3)
    B = np.array([b1, b2, b3]) / (2*np.pi)

    kpath_loc = np.zeros(npath+1)
    segment_length = np.zeros(npath)
    for ipath in range(npath):
        dk = kpath[ipath+1] - kpath[ipath]
        segment_length[ipath] = np.linalg.norm(np.matmul(dk, B))
        kpath_loc[ipath+1] = kpath_loc[ipath] + segment_length[ipath]
    ...
    if np.all(kpt == [0.0, 0.0, 0.0]):
        for iax in range(3):
            if ( dk[iax] > 0 ):
                ...
                kmax = path[0,iax]
                kmin = path[1,iax]
            kshift = math.floor(kmax - kpt[iax])
            kpt[iax] += kshift # move kpt to be close to kpath
            kshift = math.ceil(kmin - kpt[iax])
            kpt[iax] += kshift # move kpt to be close to kpath
            if ( kpt[iax]<(kmin-norm) or \
                kpt[iax]>(kmax+norm) ):
                return False
    else:    
        for iax in range(3):
